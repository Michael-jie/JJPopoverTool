...
typedef void(^BeginRemove)();

@interface ArrowView : UIView
@property (nonatomic, strong) UIColor *arrowBgColor;
@property (nonatomic, copy) BeginRemove beginRemove;
@end
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"backgroundColor"]) {
        ...
        UIColor *backgroundColor = [change valueForKey:NSKeyValueChangeNewKey];
        [self setArrowBgColor:backgroundColor];
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    for (UIView *subView in _copyPassthroughViews) {
...
+ (void)presentContentView:(UIView *)contentView pointToItem:(UIView *)item {
    ...
    // 4.监听颜色的改变
    [contentView addObserver:arrowView forKeyPath:@"backgroundColor" options:NSKeyValueObservingOptionNew context:nil];
    
    // 5.如果监听者挂掉,则移除监听者
    __weak typeof(arrowView)weakArrowView = arrowView;
    arrowView.beginRemove = ^ {
        if (weakArrowView != nil) {
            [contentView removeObserver:weakArrowView forKeyPath:@"backgroundColor" context:nil];
        }
    };
    ...
    for (UIView *subView in keyWindow.subviews) {
        if ([subView isKindOfClass:[PopoverView class]]) {
            for (UIView *subView2 in subView.subviews) {
                if ([subView2 isKindOfClass:[ArrowView class]]) {
                    ArrowView *arrowView = (ArrowView *)subView2;
                    if (arrowView.beginRemove) {
                        arrowView.beginRemove();
                    }
                }
            }
